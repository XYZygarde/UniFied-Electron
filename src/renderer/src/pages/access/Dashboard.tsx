...
function Dashboard(): React.JSX.Element {
  ...
  const [configData, setConfigData] = useState<SecureConfigData | null>(null)
  ...
  useEffect(() => {
    const fetchConfig = async (): Promise<void> => {
      ...
      if (typeof window !== 'undefined' && api) {
        try {
          const data = (await api.invoke('get-secure-config')) as SecureConfigData
          setConfigData(data)
        } catch (error) {
  ...
  const eraseConfig = async (): Promise<void> => {
    const api = (window as unknown as { api?: ElectronApi }).api
    if (typeof window !== 'undefined' && api) {
      try {
        await api.invoke('erase-config')
        setConfigData(null)
        navigate('/')
      } catch (error) {
        console.error('Error erasing secure config:', error)
      }
    }
  }
  ...
  return (
    <main className="flex min-h-screen w-screen flex-col items-center bg-[#111625] p-6 sm:p-10">
      ...
      <section className="w-full max-w-4xl flex-col rounded-[16px] border border-white/20 bg-white/5 p-6 shadow-lg backdrop-blur-sm">
        <h2 className="mb-4 font-poppins text-xl font-semibold text-white">
          Active System Configuration
        </h2>

        <p className="mb-4 font-dm-sans text-sm text-slate-300">
          This data is pulled dynamically from the OS-level encrypted{' '}
          <code className="rounded bg-black/40 px-1 py-0.5 text-blue-300">secure-config.enc</code>{' '}
          file.
        </p>
        ...
        <div className="relative rounded-lg border border-[#446b96] bg-[#0a0d16] p-4">
          {isLoading ? (
            ...
            <pre className="overflow-x-auto whitespace-pre-wrap rounded-md bg-[#06101d] p-3 text-xs text-green-400">
